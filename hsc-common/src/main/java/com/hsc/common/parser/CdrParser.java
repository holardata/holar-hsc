package com.hsc.common.parser;

import cn.hutool.core.net.URLDecoder;
import com.alibaba.fastjson.JSON;
import com.hsc.common.domain.*;
...
public class CdrParser {
    ...
    public static Cdr decodeThenParse(String reqText) throws ParserException {
        String decodeText = URLDecoder.decode(reqText, Charset.defaultCharset());
    ...
    private static void assignHoldElement(final List<Hold> holds, final Element rootElement) {
        ...
        attributes(rootElement, (name, value) -> {
            switch (name) {
                case ON:
                    hold.setOn(NumberUtils.toLong(value));
                    break;
                case OFF:
                    hold.setOff(NumberUtils.toLong(value));
                    break;
                case BRIDGED_TO:
        ...
        attributes(rootElement, (name, value) -> {
            switch (name) {
                ...
                case APP_DATA:
                    ...
                    break;
                case APP_STAMP:
                    application.setAppStamp(NumberUtils.toLong(value));
                    break;
                default:
    ...
    private static void assignTimesElement(Times times, Element rootElement) {
        elements(rootElement, (name, element) -> {
            ...
            Long value = NumberUtils.toLong(value0);
            switch (name) {
                case CREATED_TIME:
